...
def save_session_summary(origin, destination, direct, layovers, top_routes, optimal):
    ...
    with open(response_file, "w", encoding="utf-8") as f:
        ...
        for r in top_routes[:5]:
            f.write(
                f"{r['id']} | ${r['price']} (Base: ${r['base']} + Taxes/Fees: ${r['taxes']}) | "
                f"Stops: {r['stops']} | {r['segments']}\n"
            )

        f.write("\nOptimal Redemption:\n")
        f.write(
            f"{optimal['id']} | ${optimal['price']} (Base: ${optimal['base']} + Taxes/Fees: ${optimal['taxes']}) | "
            f"Stops: {optimal['stops']} | Segments: {optimal['segments']}\n"
        )
